...
class Webhook
{
    ...
    private function checkIsModuleActivated()
    {
        ...
        if (!$this->getModuleParam('type')) {
            $this->failProcess('Blockpen Commerce module not activated');
        }
    }
    ...
    private function getEvent()
    {
        ...
        try {
            $event = \BlockpenCommerce\Webhook::buildEvent($payload, $signatureHeader, $secretKey);
    ...
    private function getCharge($chargeId)
    {
        ...
        // \BlockpenCommerce\ApiClient::init($apiKey);

        try {
            $charge = \BlockpenCommerce\Resources\Charge::retrieve($chargeId);
        } catch (\Exception $exception) {
            $this->failProcess($exception->getMessage());
        }

        if (!$charge) {
            $this->failProcess('Charge was not found in Blockpen Commerce.');
        }
